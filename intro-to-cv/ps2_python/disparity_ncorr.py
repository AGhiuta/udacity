...
def disparity_ncorr(L, R, tplSize=[11, 11], disparity=30):
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            err = cv2.matchTemplate(RStrip, tpl, method=cv2.TM_CCORR_NORMED)
            err = cv2.normalize(err, err, alpha=0, beta=1,
                                norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)

            # Compute the disparity
            _, _, _, maxLoc = cv2.minMaxLoc(err)
            D[row, col] = maxLoc[0] + RStripMinCol - col
